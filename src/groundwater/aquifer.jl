...
transmissivity).
"""
@get_units @bmi_land_grid struct ConfinedAquifer{T} <: Aquifer
...
(Johnson, 1967).
"""
@get_units @bmi_land_grid struct UnconfinedAquifer{T} <: Aquifer
    head::Vector{T} | "m"  # hydraulic head [m]
...
@get_units @bmi_land_grid struct ConstantHead{T}
    head::Vector{T} | "m"
    index::Vector{Int} | "-"
end
