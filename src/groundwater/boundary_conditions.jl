...
@get_units @bmi_river_grid struct River{T} <: AquiferBoundaryCondition
    stage::Vector{T} | "m"
...
@get_units @bmi_drain_grid struct Drainage{T} <: AquiferBoundaryCondition
...
@get_units @bmi_land_grid struct Recharge{T} <: AquiferBoundaryCondition
    rate::Vector{T} | "m d-1"
    flux::Vector{T} | "m3 d-1"
    index::Vector{Int} | "-"
end
